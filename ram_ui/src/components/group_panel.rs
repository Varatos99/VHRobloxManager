...
pub enum GroupPanelAction {
    ...
    /// Launch Roblox without a specific game for all selected accounts.
    BulkLaunchRoblox,
    /// Deselect all.
    ClearSelection,
...
pub fn show(
    ...
    roblox_running: bool,
    anonymize: bool,
    favorite_places: &[ram_core::models::FavoritePlace],
        ...
        ui.vertical(|ui| {
                ...
                .show(ui, |ui| {
                    for (idx, account) in selected_accounts.iter().enumerate() {
                        ui.horizontal(|ui| {
                            ...
                            let (dot_rect, _) = ui
                                .allocate_exact_size(egui::vec2(10.0, 14.0), egui::Sense::hover());
            ...
            egui::Grid::new("bulk_launch_grid")
                ...
                .show(ui, |ui| {
                    ...
                    ui.label("Favorite:");
                    egui::ComboBox::from_id_salt("favorite_dropdown")
                        .selected_text(
                            state
                                .selected_favorite
                                .map(|i| format!("Use Favorite"))
                                .unwrap_or_else(|| "Select...".to_string()),
                        )
                        .show_ui(ui, |ui| {
                            ui.selectable_value(&mut state.selected_favorite, None, "Select...");
                            for (idx, fav) in favorite_places.iter().enumerate() {
                                if ui
                                    .selectable_value(
                                        &mut state.selected_favorite,
                                        Some(idx),
                                        &fav.name,
                                    )
                                    .clicked()
            ...
            ui.horizontal(|ui| {
                ...
                // Launch Roblox only (no game) button
                if ui.button("\u{25b6}  Launch Roblox").clicked() {
                    action = Some(GroupPanelAction::BulkLaunchRoblox);
                }

                if roblox_running && ui.button("\u{2620}  Kill All Instances").clicked() {
